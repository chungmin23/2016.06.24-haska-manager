...
public class StudentSearch extends HttpServlet {
	...
	protected void studentsearchSql(){
		sql="select no,age,name,hakbun from student where name='"+nameSearch+"'";
		
	}
	protected ResultSet studentSearchExecter()throws Exception{//실행 -6
		 rs =StudentServlet.stmt.executeQuery(sql);//스튜던트 서블릿에서 받아옴
		return rs;
	}
	protected void studentSearchDisp()throws Exception{
		StudentServlet.out.print("<h1>학생검색 </h1>");
		...
		while(rs.next()){
			...
			StudentServlet.out.print(hakbun+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;");
		}
