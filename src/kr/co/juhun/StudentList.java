...
public class StudentList extends HttpServlet {
	...
	protected void studentListSql(){
		sql="select age,name,hakbun from student";
		
	}
	protected ResultSet studentListExecter()throws Exception{//resultset 전체적으로 할때
		 rs =StudentServlet.stmt.executeQuery(sql);
		return rs;
	}
	protected void studentListDisp()throws Exception{
		StudentServlet.out.print("<h1>학생 전체출력</h1>");
		StudentServlet.out.print("<img src='img/juhun.png'>");
		StudentServlet.out.print("<fieldset>");
		StudentServlet.out.print("<legend>학생전체출력</legend>");
		StudentServlet.out.print("나이 이름 학번<br>");
		while(rs.next()){
			int age = rs.getInt("age");
			String name = rs.getString("name");
			String hakbun = rs.getString("hakbun");
			StudentServlet.out.print(age+"&nbsp;"+name+"&nbsp"+hakbun+"<br>");
		}
		StudentServlet.out.print("</fieldset>");
