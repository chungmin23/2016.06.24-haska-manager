...
public class ManagerList extends HttpServlet {
	...
	protected void managerListSql(){
		sql="select age,name,part from manager";
		
	}
	protected ResultSet managerListExecter()throws Exception{//resultset 전체적으로 할때
		 rs =StudentServlet.stmt.executeQuery(sql);
		return rs;
	}
	protected void managerListDisp()throws Exception{
		StudentServlet.out.print("<h1>관리자 전체출력</h1>");
		StudentServlet.out.print("<img src='img/juhun.png'>");
		StudentServlet.out.print("<fieldset>");
		StudentServlet.out.print("<legend>관리자전체출력</legend>");
		StudentServlet.out.print("나이 이름 부서<br>");
		while(rs.next()){
			int age = rs.getInt("age");
			String name = rs.getString("name");
			String part = rs.getString("part");
			StudentServlet.out.print(age+"&nbsp;"+name+"&nbsp"+part+"<br>");
		}
		StudentServlet.out.print("</fieldset>");
